#!/bin/ksh
#启动农商行稽核应用
su - jhusr -c "/acmis/JHODS/JHShell/baseshell/control/startJH.sh;/acmis/ACLOG/JHLOG/listMsg.sh"
if [ $? -ne 0 ]
then
        echo "start drc jh app failed"
fi
#启动稽核村镇应用
...
#启动信贷村镇应用
su - xdczusr -c
"/acmiscz/XDCZODS/XDShell/baseshell/control/startupJKCZ.sh;/acmiscz/XDCZODS/XDShell/baseshell/control/startupXDCZ.sh"
if [ $? -ne 0 ]
then
